  ...
  function Sticky() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Sticky.__proto__ || Object.getPrototypeOf(Sticky)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
      ...
      if (_this.props.relative) {
        ...
        var _distanceFromTop = 0;

        var currentOffsetParent = _this.placeholder;

        while (currentOffsetParent && currentOffsetParent !== parent) {
          _distanceFromTop += currentOffsetParent.offsetTop;
          currentOffsetParent = currentOffsetParent.offsetParent;
        }

        _distanceFromTop = _distanceFromTop - parent.scrollTop;
      }
      ...
      var style = !isSticky ? {} : {
        position: 'fixed',
        top: bottomDifference > 0 ? _this.props.relative ? parent.getBoundingClientRect().top - parent.offsetParent.scrollTop + Math.max(0, distanceFromTop) : 0 : bottomDifference,
